...
void COMMON_PIXEL_SHADER(Surface S, inout PixelOutput PO)
{
    ...
    vec3 highlight_rimlight = HiglightRimlight(S, texture(base_texture, S.uv[0]).a);
    highlight_rimlight = highlight_rimlight * highlight_rimlight_color * base_color.rgb;

    vec3 shadow_rimlight = ShadowRimlight(S, texture(base_texture, S.uv[0]).a);
    shadow_rimlight = shadow_rimlight * shadow_rimlight_color * sss_color.rgb;
